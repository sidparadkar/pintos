...
//thread.c and thread.h 
  ...
  ASSERT (intr_get_level () == INTR_ON);
   ...
   thread_sleep (ticks);//fucntion will place the thread out of the running list in to a seperate list of sleeping threads. Every tick we need to check if a thread is ready to be woken up, and if so we need to move it to the ready que. Threads in this sleeping list need to be ordered in terms of sleep time.
